...
@Component({
  selector: 'app-job-reminder-email',
  imports: [ FormsModule, NgxEditorModule, CurrencyPipe, DatePipe ],
  providers: [ DatePipe, CurrencyPipe ],
  templateUrl: './job-reminder-email.component.html',
  styleUrl: './job-reminder-email.component.scss'
})
export class JobReminderEmailComponent extends ApiBase implements OnInit, OnDestroy {
  ...
  isReminderSent = signal(false);
  sentReminderStatusText = '';
  companyName = '';
  venueName = '';
  recipientName = '';
  recipientEmail = '';
  ...
  onRecipientNameChange(name: string): void {
    if (this.isReminderSent() || this.syncingSalutation) {
      return;
    }
    ...
    this.post<JobInvoiceEmailStatusInfo, SendInvoiceEmailRequest>('Jobs/SendInvoiceEmail', payload)
      ...
      .subscribe({
        next: res => {
          ...
          this.sentReminderStatusText = this.buildReminderSentStatusText(this.sentDate);
          this.isReminderSent.set(true);
          this.setEditorReadonly(true);
          GeneralService.showSuccessMessage('Reminder sent successfully');
        },
        error: () => GeneralService.showErrorMessage('Failed to send reminder email')
      });
  }

  private loadReminderEmailInfo(): void {
  ...
  private buildReminderSentStatusText(date: string | Date | null): string {
    const formattedDate = this._date.transform(date, 'dd/MM/yyyy') ?? '';
    return `Reminder sent on ${ formattedDate }`;
  }
