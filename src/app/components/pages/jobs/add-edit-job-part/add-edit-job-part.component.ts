...
export class AddEditJobPartComponent extends ApiBase implements OnInit {
  ...
  /** True when at least one required skill chip has count > 0. */
  hasSelectedJobPartSkills = signal(false);

  private readonly autoGeneratedSkillsNotesPrefix = 'Additional skilled service:';

  /** Skills Notes area: show when a skill is selected or notes already exist (e.g. from API). */
  showSkillsNotesBlock(): boolean {
    const raw = this.form.get('skillsNotes')?.value;
    const notes = typeof raw === 'string' ? raw.trim() : '';
    return this.hasSelectedJobPartSkills() || notes.length > 0;
  }
    ...
    this.get<CrewSkillListItem[]>('Crew/GetSkillList')
      ...
      .subscribe({
        next: (res) => {
          ...
          this.regenerateAdditionalSkilledServiceText();
        }
      });
  }
  ...
  /**
   * Fills `skillsNotes` from the current required-skills chips (overwrites on every chip change).
   * Format: "Additional skilled service: Name1, Name2 X{count2}, …" — X only when that skill's count > 1.
   * With no chips, clears only auto-generated lines (same prefix); keeps API / manual notes.
   */
  private regenerateAdditionalSkilledServiceText(): void {
    const items = (this.form.get('jobPartSkillsAddRequests')?.value as ChipCountItem[]) ?? [];
    const opts = this.skillListOptions();
    const labelById = new Map(opts.map((o) => [o.id, o.label]));

    const active = items.filter((i) => i.count > 0);
    if (!active.length) {
      this.hasSelectedJobPartSkills.set(false);
      const current = ((this.form.get('skillsNotes')?.value as string | null) ?? '').trim();
      if (current.startsWith(this.autoGeneratedSkillsNotesPrefix)) {
        this.form.patchValue({ skillsNotes: '' }, { emitEvent: false });
      }
      return;
    }

    this.hasSelectedJobPartSkills.set(true);

    const parts = active.map((i) => {
      const label = labelById.get(i.id)?.trim() || `Skill #${i.id}`;
      return i.count === 1 ? label : `${label} X${i.count}`;
    });

    const line = `${this.autoGeneratedSkillsNotesPrefix} ${parts.join(', ')}`;

    this.form.patchValue({ skillsNotes: line }, { emitEvent: false });
  }
  ...
  private syncCalculatedDisplayValues(): void {
    const update = (): void => {
      this.calculatedOotCost.set(this.calculateOotCost(Number(this.form.get('ootCost')?.value ?? 0)));
      this.calculatedLateShiftCost.set(this.calculateLateShiftCost(Number(this.form.get('lateShiftCost')?.value ?? 0)));
      this.calculatedPerDiem.set(this.calculatePerDiem(Number(this.form.get('perDiem')?.value ?? 0)));
    };

    for (const field of [ 'ootCost', 'lateShiftCost', 'perDiem', 'crewNumber' ]) {
  ...
  private calculatePerDiem(raw: number): number {
    if (!raw) {
      return 0;
    }
  ...
  private patchFormWithPart(part: JobPartDetailsResponse): void {
    ...
    this.regenerateAdditionalSkilledServiceText();
    this.calculatedOotCost.set(this.calculateOotCost(part.ootCost ?? 0));
    this.calculatedLateShiftCost.set(this.calculateLateShiftCost(part.lateShiftCost ?? 0));
    this.calculatedPerDiem.set(this.calculatePerDiem(part.perDiem ?? 0));
  }
